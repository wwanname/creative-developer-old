...
export default function Header() {
    ...
    return (
        <div className="fixed w-full p-3 bg-[#f4f0ea]">
            <div className="flex uppercase text-sm text-black font-normal">
                <Link className="absolute" href="/">wwan</Link>
                <div onClick={() => {setIsActived(!isActived)}} className="flex justify-center items-center gap-x-3 cursor-pointer w-full">
                    <div className={`${style.burger} ${isActived ? style.burgerActive : ""}`} />
                    <div className="h-full relative flex items-center">
                        <motion.p variants={opacity} animate={!isActived ? "open" : "closed"} className="m-0">Menu</motion.p>
                        <motion.p variants={opacity} animate={isActived ? "open" : "closed"}  className="m-0 absolute">Close</motion.p>
                    </div>
                </div>
                <div className="flex absolute fill-black gap-3 right-3">
                    <motion.p variants={opacity} animate={isActived ? "open" : "closed"}>Shop</motion.p>
            ...
            <motion.div variants={background} initial="initial" animate={isActived ? "open" : "closed"} className="bg-black opacity-50 w-full h-full absolute left-0 top-[100%]" />
        </div>
    )
}